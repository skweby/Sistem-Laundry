...
if (isset($_GET['hapus']) && $role === 'admin') {
    ...
    mysqli_query($conn, "DELETE FROM ratings WHERE id_rating = $id_hapus");
...
if ($filter_bintang >= 1 && $filter_bintang <= 5) {
    $where = "WHERE r.rating = $filter_bintang";
}

// Statistik ringkas
$q_stat = mysqli_query($conn, "SELECT COUNT(*) as total, AVG(rating) as rata FROM ratings");
...
$distribusi = [];
for ($i = 5; $i >= 1; $i--) {
    $q_d = mysqli_query($conn, "SELECT COUNT(*) as c FROM ratings WHERE rating = $i");
    $r_d = mysqli_fetch_assoc($q_d);
    $distribusi[$i] = $r_d['c'] ?? 0;
}

$query = mysqli_query($conn, "
    SELECT r.id_rating, r.rating, r.komentar, r.created_at, r.id_laundry, p.Nama, p.NoTelp
    FROM ratings r
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .komentar-cell { max-width: 320px; color: #475569; }
                ...
                <?php else: ?>
                    <?php while ($row = mysqli_fetch_assoc($query)): ?>
                        <tr>
                            ...
                            <td><?php echo bintangAdmin($row['rating']); ?></td>
                            <td class="komentar-cell"><?php echo $row['komentar'] ? htmlspecialchars($row['komentar']) : '<span style="color:#CBD5E1;">- tidak ada komentar -</span>'; ?></td>
